...
- **Auto-connect per site.** Opt a site in once (Chrome asks for that site's permission), and matching tabs connect themselves as soon as they register tools.
- **Minimal permissions.** No host access by default: the extension injects nothing anywhere until you open its popup on a tab (`activeTab`), and only auto-connect sites get a persistent per-site registration.
